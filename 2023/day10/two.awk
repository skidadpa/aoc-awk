...
}
{
    for (i = 1; i <= NF; ++i) {
        ...
        if ($i == "S") {
            if (p1) {
                aoc::data_error("start at both (" x1 "," y1 ") and (" i "," NR ")")
            }
            p1 = p2 = i SUBSEP NR
        }
    }
    if (!width) {
        ...
        aoc::data_error("expected width " width " saw " NF)
    }
    map_size += NF
}
...
END {
    ...
    if (length(can_go["S"]) != 2) {
        aoc::compute_error(length(can_go["S"]), "ways to leave start, should be 2")
    }
    for (dir = 0; dir < 4; ++dir) {
        if (dir in can_go["S"]) {
            if (DEBUG > 1) {
    ...
    if (dir >= 4) {
        aoc::compute_error("could not find two starting paths")
    }
    for (steps = 1; (p1 != p2) && (steps <= map_size / 2); ++steps) {
        ...
        if (dir == b1) {
            aoc::compute_error("path 1 moved backward")
        }
        p1 = move(p1, dir)
        b1 = back[dir]
        tubes[p1] = map[p1]
        ...
        if (dir == b2) {
            aoc::compute_error("path 2 moved backward")
        }
        p2 = move(p2, dir)
        b2 = back[dir]
        tubes[p2] = map[p2]
