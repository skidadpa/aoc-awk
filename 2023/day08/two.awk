...
$0 !~ patterns[phase] {
    aoc::data_error("expecting " patterns[phase] " in phase" phase)
}
phase == 1 {
    if (DEBUG) {
...
END {
    ...
    for (e in starts) {
        ...
        while (hits < 3) {
            ...
            if (element in targets) {
                if (stride && (stride != (steps - last_hit))) {
                    aoc::compute_error("program stride constancy assumption violated")
                }
        ...
        if ((buffer_size > 6) && (stride % buffer_size)) {
            aoc::compute_error("program stride divisibility assumption violated")
        }
        strides[e] = stride
        if (DEBUG) {
