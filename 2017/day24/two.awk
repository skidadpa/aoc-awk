#!/usr/bin/env gawk -f
@include "../../lib/aoc.awk"
function build(connect, strength, bridge,   len, components, p, used) {
    if (bridge in BRIDGES) {
        return
    }
    len = split(bridge, components, ",")
    split("", used)
    if (DEBUG) {
    ...
    if (!(len in MAX_STRENGTH) || (MAX_STRENGTH[len] < strength)) {
        MAX_STRENGTH[len] = strength
    }
    if (MAX_LENGTH < len) {
        MAX_LENGTH = len
    }
