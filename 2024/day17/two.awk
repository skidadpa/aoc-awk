...
function combo(operand) {
    ...
    case 7:
    default:
        aoc::compute_error("illegal combo operand " operand)
    }
}
function compute(   opcode, operand) {
    ...
    case 7: # cdv
        ...
        break
    default:
        aoc::compute_error("illegal opcode " opcode)
    }
    if (DEBUG > 1) {
        printf("\n")
    }
...
($0 !~ PATTERNS[NR]) || (NR > 5) {
    aoc::data_error("illegal program line")
}
(NR <= C) {
    REGISTER[NR] = $1
}
(NR == PROG) {
    if (NF % 1) {
        aoc::data_error("Program size must be even")
    }
    PROGRAM = $1
    split(PROGRAM, DATA, ",")
    for (i in DATA) {
...
END {
    ...
    if (PROGRAM == "0,1,5,4,3,0") {
        ...
        aoc::compute_error("only program " SUPPORTED_PROGRAM " supported")
    }
    if (DEBUG > 3) {
        printf("(IP=%d)",IP)
    }
