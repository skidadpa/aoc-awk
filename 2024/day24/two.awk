...
(FS == ": ") {
    if (NF == 0) {
        ...
        aoc::data_error("not an initialization sequence")
    }
    next
}
$0 !~ /^[[:lower:][:digit:]]{3} ((AND)|(OR)|(XOR)) [[:lower:][:digit:]]{3} -> [[:lower:][:digit:]]{3}$/ {
    aoc::data_error("not a gate sequence")
}
{
    GATES[$5] = $2
    A[$5] = $1
    ...
    if (match($0, /^[xy]([[:digit:]]{2}) ([[:upper:]]{3}) [xy]([[:digit:]]{2})/, INS)) {
        if (INS[1] != INS[3]) {
            aoc::compute_error("misconnected inputs not supported")
        }
        stage = 0 + INS[1]
        if (stage == 0) {
            if (INS[2] == "AND") {
                if ((stage + 1) in C) {
                    aoc::compute_error("second stage 0 carry out")
                }
                C[stage + 1] = $5
                CARRIES[$5] = stage
            } else if ($5 != OUTPUT[stage]) {
                aoc::data_error("complex stage 0 not supported")
            }
        } else {
            switch (INS[2]) {
            case "XOR":
                if (stage in R) {
                    aoc::data_error("PROGRAM ERROR: second stage " stage " R gate")
                }
                R[stage] = $5
                XY_XORS[$5] = stage
                break
            case "AND":
                if (stage in S) {
                    aoc::data_error("PROGRAM ERROR: second stage " stage " S gate")
                }
                S[stage] = $5
                XY_ANDS[$5] = stage
                break
            default:
                aoc::data_error("PROGRAM ERROR: complex stage " stage " not supported")
            }
        }
    }
    GATE_TYPES[$2][$5] = 1
...
END {
    ...
    for (step = 0; step < NR; ++step) {
        ...
        for (gate in READY[step]) {
            ...
            case "XOR":
                ...
                break
            default:
                aoc::compute_error("unknown gate type " GATES[gate])
            }
            if (DEBUG > 7) {
                print "computed", gate
            }
    ...
    if ((length(WAITING_A) > 0) || (length(WAITING_B) > 0)) {
        aoc::compute_error("some gates could not be processed")
    }
    z = 0
    for (i in OUTPUT) {
        if (DEBUG > 4) {
