...
BEGIN {
    ...
    PAINT_COLOR = 2
    MOVE_ROBOT = 3
    split("SCAN_COLOR PAINT_COLOR MOVE_ROBOT", TASKS)
    FS = ","
    if (DEBUG > 14) {
        print "OPCODES:" > DFILE
        for (o in OPCODE) {
...

{
    ...
    while (STATE != DONE) {
        ...
        if (STATE == NEED_INPUT) {
            if (TASK != SCAN_COLOR) {
                aoc::compute_error("at task " TASKS[TASK] " but robot asked for input")
            }
            if ((X SUBSEP Y) in HULL) {
                INPUT = HULL[X,Y]
            } else {
                INPUT = BLACK
            }
            ...
            TASK = PAINT_COLOR
            if (DEBUG > 1) {
                print "SCANNED", COLORS[INPUT] > DFILE
            }
        }
        if (STATE == GAVE_OUTPUT) {
            if ((OUTPUT != 0) && (OUTPUT != 1)) {
                aoc::compute_error("output must be 0 or 1, got " OUTPUT)
            }
            if (TASK == PAINT_COLOR) {
                HULL[X,Y] = OUTPUT
                XMIN = aoc::min(X, XMIN)
                ...
                TASK = MOVE_ROBOT
                if (DEBUG > 1) {
                    print "PAINT", COLORS[OUTPUT] > DFILE
                }
                if (DEBUG > 2) {
                    for (y = YMIN; y <= YMAX; ++y) {
                        for (x = XMIN; x <= XMAX; ++x) {
                            if (((x SUBSEP y) in HULL) && HULL[x,y]) {
                                printf "#"
                            } else {
                                printf "."
                            }
            ...
            } else if (TASK == MOVE_ROBOT) {
                DIR = TURN[DIR][OUTPUT]
                if (DEBUG > 1) {
                    printf "TURN %s and MOVE %s", TURNS[OUTPUT], DIRECTIONS[DIR] > DFILE
                    printf " from [%d,%d]", X, Y > DFILE
                }
                X += DX[DIR]
                Y += DY[DIR]
                if (DEBUG > 1) {
                ...
                aoc::compute_error("at task " TASKS[TASK] " but robot provided output")
            }
        }
    }
    for (y = YMIN; y <= YMAX; ++y) {
