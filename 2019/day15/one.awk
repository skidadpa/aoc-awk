...
function visit_to_distance(type, moves,   ox, oy, result, dir, x, y, dest, move_status, sub_result) {
    ox = XDROID
    oy = YDROID
    VISITED[ox, oy] = 1

    if (DEBUG) {
        printf "visit [%d,%d] of type %s to distance %d\n", ox, oy, sFOUND[type], moves > DFILE
    }

    if ((moves < 1) || (type != EMPTY)) {
        return type
    }
    ...
    for (dir = N; dir <= E; ++dir) {
        ...
        y = oy + DY[dir]
        dest = x SUBSEP y
        if ((dest in VISITED) || (dest in WALLS) || (dest in DEAD_ENDS)) {
            continue
        }
        move_status = move_droid(dir, dest)
        if (move_status != WALL) {
            XDROID = x
            YDROID = y
            sub_result = visit_to_distance(move_status, moves - 1)
            result = aoc::max(result, sub_result)
            move_status = move_droid(RETURN_PATH[dir], "")
            if (move_status != EMPTY) {
                aoc::compute_error("expected EMPTY hall on return, got " sFOUND[move_status])
            }
            XDROID = ox
            YDROID = oy
            if (result == OXYGEN) {
...
function distance_to_oxygen(   moves) {
    for (moves = 1; moves <= MAX_MOVES; ++moves) {
        ...
        split("", VISITED)
        if (XDROID || YDROID) {
            aoc::compute_error("droid reset to [" XDROID "," YDROID "] instead of [0,0]")
        }
        if (visit_to_distance(EMPTY, moves) == OXYGEN) {
            if (VIDEO) {
                show_map()
            }
            return moves
    ...
    aoc::compute_error("no solution found in " MAX_MOVES " moves")
}
{
    split("", MEM)
    for (i = 1; i <= NF; ++i) {
