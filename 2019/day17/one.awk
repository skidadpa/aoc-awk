...
}
{
    ...
    while (1) {
        ...
        case ">":
            SCAFFOLD[x,y] = 1
            if (FACING != "") {
                aoc::compute_error("multiple robots detected")
            }
            FACING = out
            break
        default:
            aoc::compute_error("got unexpected output " out)
        }
        if (out == "\n") {
            ++y
            x = 0
    ...
    for (loc in SCAFFOLD) {
        ...
        x = c[1]
        y = c[2]
        if ((((x-1) SUBSEP y) in SCAFFOLD) && (((x+1) SUBSEP y) in SCAFFOLD) &&
            ((x SUBSEP (y-1)) in SCAFFOLD) && ((x SUBSEP (y+1)) in SCAFFOLD)) {
            alignment_parameter = x * y
            INTERSECTIONS[x,y] = alignment_parameter
