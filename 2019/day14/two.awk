...
function calculate_ore(fuel_required,   ore_required, material, STEPS, step, output_needed, multiplier, output_produced, extra_output, input_needed) {
    ore_required = 0
    for (material in STORES) {
        STORES[material] = 0
    }
    split("", STEPS)
    step = 0
    STEPS[step]["FUEL"] = fuel_required
    while (step in STEPS) {
        if (DEBUG > 1) {
            printf "STEP %d:\n", step > DFILE
        }
    ...
    if (DEBUG) {
        printf "%d ore required for %d FUEL\n", ore_required, fuel_required > DFILE
    }
    return ore_required
}
...
END {
    ...
    known_good = int(ore_available / calculate_ore(1))
    known_bad = known_good * 2
    if (DEBUG) {
        printf "%d ore available\n", ore_available > DFILE
    }
    while ((known_good + 1) < known_bad) {
        try_fuel = int((known_good + known_bad) / 2)
        if (calculate_ore(try_fuel) <= ore_available) {
            known_good = try_fuel
        } else {
            known_bad = try_fuel
        }
