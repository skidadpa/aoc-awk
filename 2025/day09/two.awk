...
function link_tiles(a, b,   direction, edge) {
    if (X[a] != X[b]) {
        if (Y[a] != Y[b]) {
            aoc::data_error("adjacent tiles " a " and " b " not inline")
        }
        if (X[a] > X[b]) {
            link_tiles(b,a)
            return
        ...
        if (Y[a] == Y[b]) {
            aoc::data_error("adjacent tiles " a " and " b " at same location")
        }
        if (Y[a] > Y[b]) {
            link_tiles(b,a)
            return
...
END {
    ...
    while (i != START) {
        ...
        if (next_move_vertical) {
            ...
            case -4: # NOT_SW
                ...
                break
            default:
                aoc::compute_error("illegal corner type " last_corner)
            }
        } else {
            left = (X[n] < X[i])
            switch (last_corner) {
        ...
        if (!last_corner) {
            ...
            aoc::compute_error("clockwise interior calculation failed")
        }
        if (DEBUG > 4) {
    ...
    aoc::compute_error("no solutions found")
}
