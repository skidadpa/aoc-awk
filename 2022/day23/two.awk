...
function select_direction(x, y,   i, m) {
    if (((x - 1, y - 1) in MAP) ||
        ...
        for (i in MOVES) {
            m = MOVES[i]
            if (DEBUG > 2) {
                print "Looking", m, "from", x, y > DFILE
                print x + CHECK_X[m][-1], y + CHECK_Y[m][-1], ((x + CHECK_X[m][-1], y + CHECK_Y[m][-1]) in MAP) ? "OCCUPIED" : "EMPTY" > DFILE
                print x + CHECK_X[m][0], y + CHECK_Y[m][0], ((x + CHECK_X[m][0], y + CHECK_Y[m][0]) in MAP) ? "OCCUPIED" : "EMPTY" > DFILE
                print x + CHECK_X[m][1], y + CHECK_Y[m][1], ((x + CHECK_X[m][1], y + CHECK_Y[m][1]) in MAP) ? "OCCUPIED" : "EMPTY" > DFILE
            }
...
function round(   e, coords, x, y, dir, PROPOSED_MOVES, DUPLICATES, m, moves) {
    ...
    for (e in MAP) {
        ...
        if (DEBUG > 2) print "elf at", x, y, "wants to move in direction", dir > DFILE
        if (dir != NO_MOVE) {
            x += CHECK_X[dir][0]
            y += CHECK_Y[dir][0]
    ...
    if (DEBUG) {
        ...
        moves = moves ")"
        print length(PROPOSED_MOVES), "proposed moves", moves, "with", length(DUPLICATES), "duplicates" > DFILE
    }
    if (DEBUG > 1) {
        compute_bounds()
        for (y = YMIN - 2; y <= YMAX + 2; ++y) {
            for (x = XMIN - 2; x <= XMAX + 2; ++x) {
                if ((x,y) in MAP) printf("#") > DFILE
                else if ((x,y) in DUPLICATES) printf("0") > DFILE
                else if ((x,y) in PROPOSED_MOVES) printf("o") > DFILE
                else printf(".") > DFILE
            }
            printf("\n") > DFILE
        }
...
END {
    ...
    compute_bounds()
    if (DEBUG) {
        print "AT START area =", AREA, "elves =", ELVES, "empty =", (AREA - ELVES) > DFILE
        compute_bounds()
        for (y = YMIN - 2; y <= YMAX + 2; ++y) {
            for (x = XMIN - 2; x <= XMAX + 2; ++x) {
                if ((x,y) in MAP) printf("#") > DFILE
    ...
    for (round_number = 1; round_number <= ROUND_LIMIT; ++round_number) {
        if (DEBUG) print "ROUND", round_number > DFILE
        if (round()) {
            break
        }
        if (DEBUG) {
            compute_bounds()
            print "area =", AREA, "empty =", (AREA - ELVES) > DFILE
        }
    }
    if (DEBUG) {
        print "AT END area =", AREA, "empty =", (AREA - ELVES) > DFILE
        compute_bounds()
        for (y = YMIN - 2; y <= YMAX + 2; ++y) {
            for (x = XMIN - 2; x <= XMAX + 2; ++x) {
                if ((x,y) in MAP) printf("#") > DFILE
    ...
    if (round_number > ROUND_LIMIT) {
        aoc::compute_error(round_number " exceeds round limit " ROUND_LIMIT)
    }
    print round_number
}
