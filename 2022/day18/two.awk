...
END {
    if (DEBUG) print length(CUBES), "cubes, finding exterior in x", XMIN, XMAX, "y", YMIN, YMAX, "z", ZMIN, ZMAX > DFILE
    ...
    if (DEBUG) print length(EXTERIOR), "exterior out of", length(CHECKED), "checked points, searching from Z ends" > DFILE
    for (x = XMIN; x <= XMAX; ++x) for (y = YMIN; y <= YMAX; ++y) {
        look_for_external(x,y,ZMIN)
        look_for_external(x,y,ZMAX)
    }
    if (DEBUG) print length(EXTERIOR), "exterior out of", length(CHECKED), "checked points, searching from Y ends" > DFILE
    for (x = XMIN; x <= XMAX; ++x) for (z = YMIN; z <= YMAX; ++z) {
        look_for_external(x,YMIN,z)
        look_for_external(x,YMAX,z)
    }
    if (DEBUG) print length(EXTERIOR), "exterior out of", length(CHECKED), "checked points, searching from X ends" > DFILE
    for (y = YMIN; y <= YMAX; ++y) for (z = YMIN; z <= YMAX; ++z) {
        look_for_external(XMIN,y,z)
        look_for_external(XMAX,y,z)
    }
    if (DEBUG) print length(EXTERIOR), "exterior out of", length(CHECKED), "checked points, finding exposed sides" > DFILE
    for (c in CUBES) {
