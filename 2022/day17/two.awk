...
function draw_map(top,   x, y) {
    for (y = top; y >= 0; --y) {
        for (x = 0; x <= 8; ++x) {
            printf("%c", ((x,y) in MAP) ? MAP[x,y] : ".") > DFILE
        }
        printf("\n") > DFILE
    }
    printf("\n") > DFILE
...
}
{
    ...
    if (DEBUG) {
        print "num gusts", num_gusts > DFILE
        print "repeat from", start, "to", end, "of shape", top_shape > DFILE
        print "repeat count", size, "interval", repeat_interval > DFILE
        print "start height", HEIGHTS[start], "repeat height", repeat_height > DFILE
        if (DEBUG > 1) {
            for (i = 0; i < size; ++i) {
                print "remainder", i, ":", REMAINDER_ADD[i] > DFILE
            }
        }
        print "num repeats", num_repeats, "remainder", remainder > DFILE
    }
