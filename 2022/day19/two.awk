...
function find_most_geodes(path, nxt,   builds, time, M, R, geodes, t, m, r) {
    ...
    if (DEBUG) printf(" %d (%d)", geodes, most_geodes) > DFILE
...
}
{
    ...
    if (DEBUG) {
        print "Blueprint", $1 > DFILE
        for (r in C) {
            printf("  %s robot costs:",r) > DFILE
            for (m in C[r]) {
                printf(" %d %s", C[r][m], m) > DFILE
            }
            printf("\n") > DFILE
        }
        for (m in MAX_USABLE) {
            print " max", MAX_USABLE[m], m, "robots usable" > DFILE
        }
    }
    most_geodes = 0
    if (MAX_USABLE[ORE] > 1) {
    ...
    if (DEBUG) {
        print "Blueprint", $1, "results: quality", $1 * most_geodes > DFILE
    }
