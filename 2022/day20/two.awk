...
END {
    ...
    if (DEBUG) {
        ...
        for (i in TAKE_FROM) {
            printf(" %d", FILE[TAKE_FROM[i]]) > DFILE
        }
        printf("\n") > DFILE
    }
    for (iteration = 1; iteration <= ITERATIONS; ++iteration) {
        for (n in FILE) {
            ...
            if (DEBUG > 1) {
                printf("move %d from %d to %.1f:", FILE[n], old_pos, new_pos) > DFILE
                for (i in TAKE_FROM) {
                    printf(" %d", FILE[TAKE_FROM[i]]) > DFILE
                }
            }
        ...
        if (DEBUG) {
            printf("after iteration %d:", iteration) > DFILE
            for (i in TAKE_FROM) {
                printf(" %d", FILE[TAKE_FROM[i]]) > DFILE
            }
            printf("\n") > DFILE
    ...
    if (DEBUG) {
        print "zero at:", zero_pos > DFILE
        print "coordinates at:", cp1, cp2, cp3 > DFILE
        print "coordinates:", c1, c2, c3 > DFILE
    }
    print c1 + c2 + c3
}
